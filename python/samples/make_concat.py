...
for each in sequence:
    real_file_name = each['src'] if '.mp4' in each['src'] else each['src'] + '.mp4'
    if 'start' not in each:
        file_name_list.append(real_file_name)
    else:  # need to trim
        result_file_name = '%s-%s-%s.mp4' % (each['src'].split('-')[0], each['start'], each['end'])
        my_h264_worker.trim(real_file_name, each['start'], each['end'], result_file_name, 30)
