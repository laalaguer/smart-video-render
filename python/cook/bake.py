...
# ['layer','layer','layer'] --> the left side is the bottom, the right side is the upper
# layer {
...
def generate_filtergraph(cake, outputpad):
	...
	for idx, each in enumerate(cake):
		...
		temp_str_filters.extend([filter_adapter(every) for every in each['filters']])
		temp_chain = chain.get_single_chain([str(idx)],[temp_prefix+str(idx)],temp_str_filters)
