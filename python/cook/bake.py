...
def generate_cake_render_command(cake, result_file_name, codecname='libx264', codecflag='-crf 18 -preset slow', ffmpeg_location=None, progressfilename=None, overwriteflag='-y'):
    ...
    temp_bucket = [program, debug_switch, overwriteflag, progress_switch,
...
def generate_concat_command(parts, result_file_name):
    ...
    '''
    program = 'ffmpeg -y -v error -f concat'
    try_list_file = _generate_concat_files_list_option(parts, 'temp_join_list.txt')
    if try_list_file:
...
def generate_trim_command(input_file_name, start_time, end_time, result_file_name, codecname='libx264', codecflag='-crf 18 -preset slow'):
    ...
    return 'ffmpeg -y -v error -i %s -ss %s -vcodec %s %s -to %s %s' % (
        input_file_name, start_time, codecname, codecflag, end_time, result_file_name)
